...
- `START`后先以50 Hz开启加速度计和陀螺仪，确保启动后的第一步可以被捕获；温度传感器始终关闭。
- 连续静止3秒后自动关闭陀螺仪，仅保留25 Hz低功耗加速度计；连续检测到活动80 ms后重新开启50 Hz陀螺仪。
- 加速度计在静止门控阶段使用低功耗模式，陀螺仪只在活动阶段使用低噪声模式。
- 静止时MCU每40 ms唤醒并只读取6字节加速度；活动时每20 ms读取12字节加速度和角速度，处理结束立即睡眠。
- 计步只使用陀螺仪：自动选择角速度最大的轴，同一轴出现方向相反且均超过40 dps的两个运动波瓣，计为一次完整往复。
- 每个方向必须连续保持至少3个采样点（60 ms），正反转至少间隔100 ms，计步后有240 ms防重复时间，轻微抖动和单点尖峰不会计步。
